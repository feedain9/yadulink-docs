---
sidebar_position: 8
title: Créer un template de message
---

# Créer un template de message

1. Rends-toi sur **Yadulink > Messages privés > Création de template**

2. Donne un nom à ton template, il sera utile pour ton organisation

3. Rédige ton template en utilisant la variable `{{first_name}}`, l'IA s'occupera de la remplir pour toi lors de l'utilisation

4. Une fois terminé, clique sur **Enregistrer**
